...
from models_nointernet import MLPModel, GCNModel, AttentionModel
...
def main():
    ...
    for epoch in range(epochs):
        ...
        running_acc = 0.0
        model.train()
        for i, d in enumerate(tqdm(training_generator, desc=f"Epoch {epoch+1}/{epochs}")):
            batch, labels = d
            # Transfer to GPU
        ...
        with torch.set_grad_enabled(False):
            ...
            if val_losses[-1] == min_loss:
                torch.save(model.state_dict(), os.path.join(output_path, 'weights_pretrained.{epoch:02d}-{min_loss:.2f}.pt'.format(epoch = epoch+1, min_loss = min_loss)))
    ...
    torch.save(model.state_dict(), os.path.join(output_path, "final_weights."+str(epochs)+".pt"))
